...
// View credentials for the logged-in user -- tested and works
...
// Fetch credentials based on OU or division -- tested and works
router.get('/credentials/:type/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (req.userRole !== 'admin') {
      return res.status(403).json({ error: 'Permission denied. Only admin users can access this route.' });
    }
...
// Add credentials -- tested and works
router.post('/credentials', verifyToken, async (req, res) => {
  try {

    let name, username, password, division, ou;
    ...
    if (req.userRole === 'admin') {
      ...
      if (req.userOU !== ou && req.userDivision !== division) {
        return res.status(403).json({ error: 'Permission denied. You can only add credentials to your assigned OU and division.' });
      }
    }
    ...
    res.status(500).json({ error: 'Failed to create a new credential', details: error.message });
  }
});

// Update an existing credential -- tested and works
router.put('/credentials/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to update the credential', details: error.message });
  }
});
