...
router.get('/user-data', verifyToken, async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select('username role ou division');
...
// Fetch a list of users for admin-- tested and works
router.get('/users', verifyToken, async (req, res) => {
  try {
    // Check if the user making the request is an admin
    if (req.userRole !== 'admin') {
      return res.status(403).json({ error: 'Permission denied. Only admin users can access the list of users.' });
    }
