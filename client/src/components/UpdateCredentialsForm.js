// client/src/components/UpdateCredentialsForm.js
...
function UpdateCredentialsForm({ credentials, userRole, userOU, userDivision }) {
  ...
  const handleUpdateCredential = async (e) => {
    ...
    try {
      ...
      if ( // admin got access to all, management only to their division and ou
        userRole === 'admin' ||
        (userRole === 'management' &&
          userOU === selectedCredential.ou &&
          userDivision === selectedCredential.division)
      ) { // Make a PUT request
